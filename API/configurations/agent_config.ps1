...
cd $env:SystemDrive\'azagent'; for($i=1; $i -lt 100; $i++)
{
    $destFolder="A"+$i.ToString();
    if(-NOT (Test-Path ($destFolder))){mkdir $destFolder;cd $destFolder;break;}
}; 
$agentZip="$PWD\agent.zip";$DefaultProxy=[System.Net.WebRequest]::DefaultWebProxy;$securityProtocol=@();
...
; $Uri='https://vstsagentpackage.azureedge.net/agent/2.194.0/vsts-agent-win-x64-2.194.0.zip';
...
.\config.cmd --deploymentgroup --deploymentgroupname "QoS Services Group Deploy" --agent $env:COMPUTERNAME --runasservice --work '_work' --url 'https://dev.azure.com/VodafoneApps/' --projectname 'QCI'; 
